...
execSync(
  "rm -rf ../../mods/cmds/cache && mkdir -p ../../mods/cmds/cache && rm -rf ../../mods/events/cache && mkdir -p ../../mods/events/cache && rm -rf cache && mkdir -p cache ",
  (error, stdout, stderr) => {
    if (error) {
...
try {
  var appStateFile = resolve(
    join(
      ...
      config.APPSTATEPATH || "../../facebookstate.json",
    ),
  );
  var appState =
...
function onBot({ models: botModel }) {
  ...
  login(loginData, async (loginError, loginApiData) => {
    ...
    (global.config.version = config.version),
        ...
        for (const command of listCommand) {
          try {
            ...
            if (!config?.commandCategory) {
              try {
                throw new Error(
                  `[ 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 ] • ${command} commandCategory is not in the correct format or empty`,
                );
              } catch (error) {
            ...
            try {
              global.loading(
                `${crayon(``)}successfully loaded ${chalk.blueBright(
                  config.name,
                )}`,
                "[ 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 ] •",
              );
            ...
            global.loading.err(
              `${chalk.hex("#ff7100")(``)}failed to load ${chalk.hex("#FFFF00")(
                command,
              )}` + error,
              "[ 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 ] •",
            );
      ...
      (async () => {
        const events = readdirSync(
          join(global.client.mainPath, "../../mods/events"),
        ).filter(
          (ev) =>
        ...
        for (const ev of events) {
          try {
            const event = require(
              join(global.client.mainPath, "../../mods/events", ev),
            );
    ...
    global.loading(
      `${crayon(``)}loaded ${chalk.blueBright(
...
(async () => {
  try {
    ...
    logger("successfully connected to database.", "[ 𝙳𝙰𝚃𝙰𝙱𝙰𝚂𝙴 ] •");
  }
})();
